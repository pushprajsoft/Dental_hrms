...
@section('title', 'Add Bed Master')
@section('page-title', 'Add New Bed')
@section('page-subtitle', 'Register a new bed or room into the hospital system')
...
<div class="bed-form-container">
    ...
    <div class="bed-card">
        ...
        <div class="bed-card-body">
            <form action="{{ route('ipd.beds.store') }}" method="POST">
                @csrf
                <div class="bed-form-grid">
                    <div class="bed-form-group">
                        <label>Bed Number *</label>
                        <input type="text" name="bed_number" class="bed-input" placeholder="e.g. B-101" value="{{ old('bed_number') }}" required>
                    </div>
                    <div class="bed-form-group">
                        <label>Room Number</label>
                        <input type="text" name="room_number" class="bed-input" placeholder="e.g. 101" value="{{ old('room_number') }}">
                    </div>
                    <div class="bed-form-group">
                        <label>Bed Type *</label>
                        <select name="bed_type" class="bed-input" required>
                            <option value="General" @selected(old('bed_type') == 'General')>General</option>
                            <option value="Private" @selected(old('bed_type') == 'Private')>Private</option>
                            <option value="ICU" @selected(old('bed_type') == 'ICU')>ICU</option>
                            <option value="Ward" @selected(old('bed_type') == 'Ward')>Ward</option>
                        </select>
                    </div>
                    <div class="bed-form-group">
                        <label>Charge Per Day (₹) *</label>
                        <input type="number" name="charge_per_day" class="bed-input" placeholder="e.g. 500" step="0.01" value="{{ old('charge_per_day', '0') }}" required>
                    </div>
                ...
                <div style="display: flex; gap: 16px; justify-content: flex-end; margin-top: 28px;">
                    ...
                    <button type="submit" class="btn-bed-submit">
                        <i class="fa-solid fa-check"></i> Save Bed
                    </button>
                </div>
            </form>
        </div>
